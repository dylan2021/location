...
public abstract class BaseMapCallImplement
    ...
    TileProcessController tileProcessCtrl = null;
    TextTextureGenerator textTextureGenerator = null;
    final Object mapGridFillLock = new Object();
    ...
    protected void proccessRequiredData(MapCore paramMapCore, ArrayList<MapSourceGridData> paramArrayList, int mapType) {
        ...
        for (int i = 0; i < paramArrayList.size(); i++) {
            ...
            if ((this.tileProcessCtrl == null) || (!this.tileProcessCtrl.hasKey(localMapSourceGridData1.getKeyGridName()))) {
                Object localObject2;
                Object localObject3;
                if (mapType == BaseMapLoader.DATA_SOURCE_TYPE_DATA_VEC_TMC) {//矢量实时交通数据
                    ...
                    try {
                        ...
                        if ((localObject3 != null) && (((Recoder) localObject3).string != null) &&
                                (((Recoder) localObject3).string.length() > 0)) {
                        }
                        localArrayList.add(localMapSourceGridData1);
                    } catch (Exception localException) {
                    }
        ...
        if (localArrayList.size() > 0) {
            ...
            for (int j = 0; j < localArrayList.size(); j++) {
                ...
                this.tileProcessCtrl.put(localMapSourceGridData2.getKeyGridName());
    ...
    public synchronized void onResume(MapCore paramMapCore) {
        try {
            ...
            this.tileProcessCtrl = new TileProcessController();
