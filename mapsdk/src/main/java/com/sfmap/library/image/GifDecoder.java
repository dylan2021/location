...
//		this.decodeModel = parseImage?DECODER_IMAGE:DECODER_CHECK_ANIMAL_ONLY;
    ...
    private boolean readHeader() throws IOException {
        if (IOUtils.startsWith(in, GIF87)) {
        } else if (IOUtils.startsWith(in, GIF89)) {
            gif89 = true;
        } else {
            return false;
        }
        // Reads Logical Screen Descriptor
        width = IOUtils.readShort(in);
        height = IOUtils.readShort(in);
        // packed fields
        int packed = in.read();
    ...
    private String readApplicationExt() throws IOException {
        int size = in.read();
        if (decodeModel == DECODER_IMAGE) {
            return new String(IOUtils.read(in, new byte[size], size));
        } else {
            IOUtils.skip(in, size);
            return null;
        }
    }
    ...
    private void readGraphicControlExt() throws IOException {
        ...
        this.currentDelay = IOUtils.readShort(in) * 10; // delay in milliseconds
