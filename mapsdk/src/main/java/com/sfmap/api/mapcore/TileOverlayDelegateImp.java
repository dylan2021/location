...
public class TileOverlayDelegateImp//bm
    ...
    public TileOverlayDelegateImp(TileOverlayOptions tileOverlayOptions, TileOverlayViewDecode tileOverlayView) {
        ...
        this.mImageFetcher.addImageCache(cacheParams);
    ...
    public void remove() // a --> remove
    {
        ...
        this.mTileOverlayView.remove(this);
    ...
    public void setVisible(boolean paramBoolean) // a
    {
...
//        Log.e("getTilesInDomain","tile_xyz:"+xIndex+"_"+yIndex+"_"+iCurZoomLevel);
    ...
    /*
    for (Iterator<TileCoordinate> localIterator1 = this.listTileDraw.iterator(); localIterator1.hasNext();)
    {
      TileCoordinate locala1 = (TileCoordinate)localIterator1.next();
      int i2 = 0;
      for (TileCoordinate locala2 : tileFound) {
        if ((locala1.equals(locala2)) && (locala1.isLoadTexture))
        {
          locala2.isLoadTexture = locala1.isLoadTexture;
          locala2.textureId = locala1.textureId;
          i2 = 1;
          break;
        }
      }
            ...
            if (tileCoordInDomain != null) {
                                ...
                                (!RegionUtil.a(tileCoordInDomain.X, tileCoordInDomain.Y, tileCoordInDomain.Zoom)))) //??????
                {
                    ...
                    if (!tileCoordInDomain.isLoadTexture) {
                        this.mImageFetcher.loadImage(needDowndload, tileCoordInDomain);
                    }
                }
            }
        }
    ...
    public void onFling(boolean isFling) //c
    {
        if (this.isFlingState != isFling) {
            this.isFlingState = isFling;
            this.mImageFetcher.setPauseWork(isFling);
        }
    }
    ...
    private class TileServer //b
        ...
        protected List<TileOverlayDelegateImp.TileCoordinate> doInBackground(IMapDelegate... mapDelegates) {
            int width;
            int height;
            try {
                width = mapDelegates[0].getMapWidth();
                height = mapDelegates[0].getMapHeight();
                this.zoom = ((int) mapDelegates[0].getZoomLevel());
            } catch (Throwable localThrowable) {
                width = 0;
                height = 0;
            }
            ...
            if ((paramBitmap != null) && (!paramBitmap.isRecycled())) {
                try {
                    ...
                    if (this.k < 3) {
                        ...
                        TileOverlayDelegateImp.this.mImageFetcher.loadImage(true, this);
                        this.k += 1;
                    }
                }
            } else if (this.k < 3) {
