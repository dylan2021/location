...
/**
 * Created by caohai on 2019/5/24.
...
public class AutoEditTextView extends android.support.v7.widget.AppCompatEditText {
    ...
    private void initData(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        downLineFocusColor = typeArray.getColor(R.styleable.AutoEditTextView_downLineFocusColor, getColor(R.color.colorMyStyle));
        typeArray.recycle();
        setBackground(null);
        initPaint();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //TODO 绘制左侧的文字,并居中显示
        if (!TextUtils.isEmpty(leftText)) {
            canvas.drawText(leftText, getScrollX()+dip2px(8), (getMeasuredHeight() - mTextPaint.getTextSize()) / 2 + mTextPaint.getTextSize()- dip2px(2), mTextPaint);
        }
        //绘制底部的线
        if (isShowDownLine) {
