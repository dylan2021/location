...
public class GLMapResManager
{
  ...
  {
    NORAML,  SATELLITE,  BUS,STREETVIEW;
  ...
  private static final String texture_background_day = "sfmap_bg_day.data";
  private static final String texture_abckground_night = "sfmap_bg_nt.data";
  private static final String texture_roadboard_day = "sfmap_rb_day.data"; //道路路牌白天资源
  private static final String texture_roadboard_night = "sfmap_rb_nt.data"; //道路路牌黑夜资源
  private static final String texture_tmc_day = "sfmap_tmc_day.data"; //TMC白天资源
  private static final String texture_tmc_night = "sfmap_tmc_nt.data";  //TMC黑夜资源
  private static final String texture_road = "sfmap_rdt.data";  //其他资源 - 道路、铁路、过街天桥
  ...
  private static final String style_day_personal = "s7051600000032.data";
  private static final String style_day_default = "s1051600000052.data";
  private static final String style_night_default = "s2051600000030.data";
  private static final String style_satelite_day_night = "s3051450262818.data";
//  private static final String style_navi_car_day = "s1051466495472.data";
  private static final String style_navi_car_day = "s4051600000030.data";
  private static final String style_navi_car_night = "s5051600000030.data";
...
//  private static final String style_navi_car_night = "style_5_5_1469088454.data";
  private static final String texture_icon_day = "i1051600000052.data";
  private static final String texture_icon_night = "i2051600000006.data";
...
//  private static final String texture_icon_night = "icons_2_5_1466998222.data";
  private int mapStyleType = 0; //0:默认样式 1:定制样式
  ...
  public void setIconsData(boolean isGLThread)
  {
    ...
    else
    {
      ...
      this.mapDelegateImp.queueGlThread(new Runnable()
      {
        public void run()
        {
    ...
    if (MapViewTime.DAY == localMapViewTime) { //白天
      if (MapViewMode.NORAML == localMapViewMode) {//地图模式
        if (MapViewModeState.NAVI_CAR == localMapViewModeState) {
          ...
          if(this.mapStyleType == 1){
            str = style_day_personal;//定制模式
          }else{
  ...
  public void setBkTexture(boolean isGLThread)
  {
    ...
    if (this.mapDelegateImp.getMapViewTime() != MapViewTime.NIGHT)
    {
      backgroundBytes = MapTilsCacheAndResManager.getInstance(this.mContext).getOtherResData(texture_background_day); //"sfmap_bg_day.data"
      topcoverBytes = MapTilsCacheAndResManager.getInstance(this.mContext).getOtherResData(texture_top_cover_day); //"3dAir_light.data"
    }
    else
    {
      backgroundBytes = MapTilsCacheAndResManager.getInstance(this.mContext).getOtherResData(texture_abckground_night);//"sfmap_bg_nt.data"
